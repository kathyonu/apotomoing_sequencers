$(document).ready(  function() {
    $('.sequence_search_completes form input#sequencetext').bind(
      'focusin',function(event) {    
        if ($('.sequence_search_completes form input#sequencetext').val() === "Enter Your Data, then tab out") {
        $('.sequence_search_completes form input#sequencetext').val("");
      }
      else if ($('.sequence_search_completes form input#sequencetext').val() === "Please Enter Your Data, then tab out") {
        $('.sequence_search_completes form input#sequencetext').val("");
      }
      else
      {
      $('.sequence_search_completes form input#sequencetext').focus();    }    }    )
      .bind(
        'focusout',function(event) {
          if (this.value != "") {
            if (this.value != "Enter data, then tab out") {
              var $newsequence = document.getElementById('sequencetext');
              var $sequencetext = $('.sequence_search_completes form input#sequencetext').serialize();
...
//              $('#search_singular').val($search_singular_return_value);
          ...
          {
            $('.sequence_search_completes form input#sequencetext').val("Please Enter Your Data, then tab out");
//            $('#search_text').val("");
//            $('#search_creation').val("");
            $('#search_complete').val("");
//            $('#search_lexigram').val("");
